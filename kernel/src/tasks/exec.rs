...
pub async fn exec_with_process(
    ...
    let elf = if let Ok(elf) = xmas_elf::ElfFile::new(&buffer) {
        ...
        return exec_with_process(task, curr_dir, String::from("busybox"), new_args, envp).await;
    };
    ...
    if let Some(header) = header {
        if let Ok(SegmentData::Undefined(_data)) = header.get_data(&elf) {
            ...
            return exec_with_process(task, curr_dir, new_args[0].clone(), new_args, envp).await;
        }
    }
